package com.czsm.Demand_Driver.helper;
...
import com.czsm.Demand_Driver.helper.acra.GMailSender;
import com.czsm.Demand_Driver.model.ProviderBooking;
import com.czsm.Demand_Driver.model.Service;
...
public class RESTClient {
                            ...
                            try {
                                ...
                                if (message.equalsIgnoreCase(context.getString(R.string.otp_sendsms_success))) {
                                    ...
                                    if (!response.isNull("services")) {
                                        ...
                                        SERVICES = jsonArrayToServicesArray2(serviceJSON);
                                    }
    ...
    private ArrayList<Service> jsonArrayToServicesArray2(JSONArray serviceJSON) {
        ArrayList<Service> services = new ArrayList<Service>();
        for (int i = 0; i < serviceJSON.length(); i++) {
            Service service = new Service();
            try {
        ...
        Service obj = services.get(0); // remember first item
        services.clear(); // clear complete list
        services.add(obj);
